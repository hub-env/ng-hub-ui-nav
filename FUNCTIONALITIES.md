...
| **Marking**           | Active entry named by the host (`activeItemId`, two-way)                  |       ❌        |
|                       | Explicit mark outranks route matching                                     |       ❌        |
|                       | `aria-current="location"` for a mark the route did not give               |       ❌        |
...
|                | Bound to a `hub-nav` (`nav`), marking entries without a URL          |       ❌        |
|                | Explicit scroll container (`scrollContainer`)                        |       ❌        |
|                | Quiet period after a click (`clickSettleMs`)                         |       ❌        |
|                | `scrollTo(sectionId, behavior)`, offset included                     |       ❌        |
